...
class probabilistic_MIL_Bayes_spvis(nn.Module):
    ...
    def forward(self, h, coords, height, width, slide_label=None, validation=False):
        ...
        mu = torch.zeros([1, (height // PATCH_SIZE + 1) * (width // PATCH_SIZE + 1)]).to(device)
        logvar = torch.zeros([1, (height // PATCH_SIZE + 1) * (width // PATCH_SIZE + 1)]).to(device)

        mu[:, coords.long()] = params[:, 0]
        logvar[:, coords.long()] = params[:, 1]

        mu = mu.view(1, height // PATCH_SIZE + 1, width // PATCH_SIZE + 1)
        logvar = logvar.view(1, height // PATCH_SIZE + 1, width // PATCH_SIZE + 1)
